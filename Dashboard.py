...
def plot_auc(data):
    fig = px.bar(data, x='Model', y='Test AUC', title='Test AUC of Models', color_discrete_sequence=custom_colors)
    st.plotly_chart(fig)
...
def plot_radar_chart(data):
    ...
    for metric in ['Test Precision', 'Test Recall', 'Test F1', 'Test AUC']:
...
def plot_f1_auc_bubble(data):
    fig = px.scatter(data, x='Test F1', y='Test AUC', size='Test Accuracy', color='Model',
                     title='F1-Score vs AUC (Size by Accuracy)')
    st.plotly_chart(fig)
